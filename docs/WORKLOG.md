...
## 2026-09-26 · 详情页开放短语/生词勾选（用户反馈：我还是无法自主选择短语）

**问题**：猜词 & 重点短语的勾选列表（勾选 → 批量加入生词本）被
`v-if="!readonly && vocabOptions.length"` 挡在智能录入页，错题详情（readonly）只有
逐词点弹窗一条路，用户无法在详情里浏览全部短语/生词并自主挑选。

**修法**（一处改动）：去掉 `!readonly` 门——勾选列表在智能录入与详情**都显示**。
`addSelected()` 走的 `/vocab/import-english` 接口与 readonly 无关，直接可用；
加入成功后原文里对应词/短语同步变红（addedWords，上一批已做）。

**验证**：单测新增一条钉住「readonly 详情显示勾选列表 + 勾短语 → kind=phrase 入库」
（Vitest 144→145）；lint/prettier/build/E2E 53 全绿；生产 8000 实测 id 189 详情页
...
## 2026-09-26 · 回炉：浮钮 Teleport 出弹窗 + 详情勾选列表下架

**用户两条反馈**：①"把这个界面删掉"——上一批在详情页开放的猜词勾选列表不要了（回退
1369e75 的 readonly 开放，恢复为仅智能录入显示）；②"看不到划词浮钮在哪"。

**浮钮看不见的真因**：详情弹窗 `.modal-panel` 带 `backdrop-filter: blur(18px)`，
它会给 `position:fixed` 的后代当**包含块**——浮钮没被裁剪也没消失，而是被钉在弹窗面板
坐标系里、又被 `.modal-backdrop` 的 `overflow-y:auto` 裁掉。修法：浮钮
`<Teleport to="body">` + z-index 1010（压过弹窗 1000）。上一批浏览器验证只查了
"DOM 里存在"，没查可视区坐标，假阳性；这次截图复验：圈选 turns out，浮钮悬在选区正上方。
