...
const allSelected = computed(
  () => vocabOptions.value.length > 0 && selected.value.length === vocabOptions.value.length,
)
...
async function addSelected() {
  if (!selected.value.length) {
    toast.warning('请先勾选要加入生词本的单词/短语')
    return
  }
...
async function importVocabItems(items) {
  ...
  toast.success(
    `已加入生词本：新增 ${r.created} 条${r.updated ? `，补充 ${r.updated}` : ''}${existing > 0 ? `，${existing} 个已存在跳过` : ''}${r.failed?.length ? `，失败 ${r.failed.length}` : ''}`,
  )
  return r
}
...
const questions = computed(() => {
  ...
  const base = {
    question: props.parsed?.question || '',
    option_a: props.parsed?.option_a || '',
    ...
    difficulty_points: props.parsed?.difficulty_points || '',
    approach: props.parsed?.approach || '',
    question_type: props.parsed?.question_type || 'choice',
  }
...
<template>
  <div class="english-panel">
        ...
        >
          <div class="ep-q-head">
            ...
            <button
              ...
              type="button"
              class="ep-q-save"
              @click="saveAsCurrentQuestion(qi)"
            >
              存为另一题
            </button>
            <span v-if="!readonly" class="ep-q-toggle">
              <button
                type="button"
                class="ep-toggle-btn"
                :class="{ active: getMark(q) === 'wrong' }"
                @click="setMark(q, 'wrong')"
              >
                错
              </button>
              <button
                type="button"
                class="ep-toggle-btn ok"
                :class="{ active: getMark(q) === 'right' }"
                @click="setMark(q, 'right')"
              >
                对
              </button>
            </span>
    ...
    <div v-if="sentences.length" class="ep-section">
      <div class="ep-section-head">
        <Icon name="list" :size="15" /><span>句子拆解 / 句型分析</span>
      </div>
      <div class="ep-passage">
        <div v-for="(s, i) in sentences" :key="i" class="ep-sent-item">
          ...
          <div v-if="s.pattern || s.structure" class="ep-sent-note">
            ...
            <div v-if="s.structure && s.structure !== s.pattern" class="ep-sent-struct">
              结构：{{ s.structure }}
            </div>
          </div>
        </div>
    ...
    <div v-if="!readonly && vocabOptions.length" class="ep-section">
      ...
      <div class="ep-vocab-list">
        <label
          v-for="o in vocabOptions"
          :key="o.key"
          class="ep-vocab-item"
          :class="{ checked: selected.includes(o.key) }"
        >
      ...
      <div class="ep-vocab-actions">
        ...
        <UiButton v-if="selectedCount" size="sm" variant="ghost" @click="selected = []"
          >清空勾选</UiButton
        >
        <span v-if="selectedCount" class="muted" style="font-size: 12px"
          >已勾选 {{ selectedCount }} 项</span
        >
      </div>
    </div>
      ...
      <div v-else-if="lookupData" class="ep-lookup">
        ...
        <div class="ep-lookup-meanings">
          <div v-for="(m, i) in lookupData.meanings" :key="i" class="ep-lookup-meaning">
            ...
            ><span class="ep-lookup-text">{{ m.meaning }}</span>
          </div>
        </div>
