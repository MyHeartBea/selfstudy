...
cd frontend && npm test                                  # Vitest 133 个；含 DOM 级交互回归（happy-dom）与全量 SFC 静态扫描（templateBindings.test.js）
...
- 测试基线：**后端 251、前端 Vitest 133、E2E 51**（workers 已在 `playwright.config.js` 钉成 2，见第 2 节），覆盖率按 CI 口径约 73%（门槛 55%）。
