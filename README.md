...
- **智能录入**：粘贴题干 AI 解析；截图走三视觉通道（DeepSeek-Vision → GLM-4.6V-Flash → Agnes×2），
  全败退回本地 OCR；支持解题要求补充与参考图；**图片一律"先看图提文字→再文本分析"**（避免超大视觉生成超时/空返回）
- **英语整篇精读**：多图（原文+选项）一次录一篇；原文**左右两栏对照**+全文翻译+逐句拆解（结构/句型）+
  点词查义（多词性）+重点短语/生词**一键全选入生词本**+**多题**（每题标对/错，错的"答题失误"重点标注），
  **整篇=一条错题**（详情用 `EnglishAnalysisPanel` 展示全篇）
...
- **真题库 + 整卷模考**：扫描真题文件夹（科目/年份/答案配对自动识别）→ 后台 AI 拆题流水线（**扫描版 PDF 自动识别**：渲染成图 → 数学/408 优先视觉模型输出准确 LaTeX、本地 OCR 兜底）→ `/papers` 卷库浏览；
  整卷模考（`mode=mock`，倒计时/自由翻题/统一判分/成绩单），**作答且错的题自动入错题本**；成绩存档入 `mock_records`（v7）
- **生词本**（英语）：闪卡快刷（认识→1/2/4/7/15/30/60 天阶梯，模糊→明天，不认识→留在队列）、
  批量导入词表、掌握度分布
- **知识点库**：标签同义归一、AI 自动总结、贴图分析（**支持一次暂存多张截图再统一分析**，同一知识点不会拆成多条）、
  服务端分页；知识笺卡片墙（科目色脊+摘要+关联标签），**点卡片任意位置直接看全文**（详情弹窗内可练习/总结/编辑）
- **公式背诵**：分类/搜索/**过卡循环背诵模式**（没记住排队尾直到全会）；**点卡片任意位置直接看公式全文**
...
- 后端 54 个测试：`cd backend && python -m unittest discover -s tests -v`（临时库，不碰真实数据）；
  前端 Vitest **31 个**：`cd frontend && npm test`（判分/筛选纯函数 + **卡片点击与知识点弹窗的 DOM 级交互回归**，环境 happy-dom）
