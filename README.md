...
- **今日复习**：间隔重复 **SM-2 简化版**（答对=上次间隔×难度系数自适应拉长、封顶 180 天，答错-0.2 重置 1 天），
  **新题优先 + 逾期轮转 + 每日配额**（默认 50 题/天，可 `.env` 调；今天没轮到的题按"最久没碰过"往后排，**不淘汰、无毕业**），
...
- **真题库 + 整卷模考**：扫描真题文件夹（科目/年份/**角色（试卷/答案册/题+答案合卷）**自动识别，**按科目+年份去重合并**）→ 后台 AI 拆题流水线（**扫描版 PDF 自动识别**：渲染成图 → 数学/408 优先视觉模型输出准确 LaTeX、本地 OCR 兜底）→ `/papers` 卷库浏览；
...
- **知识点库**：标签同义归一、AI 自动总结、贴图分析（**支持一次暂存多张截图再统一分析**，同一知识点不会拆成多条）、
  服务端分页；知识笺卡片墙（科目色脊+摘要+关联标签），**点卡片任意位置直接看全文**
  （详情弹窗内可练习/总结/编辑，并展示**关联错题 + 掌握情况**：几题、平均掌握度、累计答错、今天到期、从未复习，支持逐题直练）
