...
def create_formula(
    ...
    conn.commit()
    row = conn.execute("SELECT * FROM formula_items WHERE id = ?", (cur.lastrowid,)).fetchone()
...
def update_formula(
    ...
    """更新公式条目。"""
    row = conn.execute("SELECT 1 FROM formula_items WHERE id = ?", (formula_id,)).fetchone()
    if row is None:
        return None
    conn.execute(
    ...
    conn.commit()
    updated = conn.execute("SELECT * FROM formula_items WHERE id = ?", (formula_id,)).fetchone()
