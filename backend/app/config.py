...
def _load_env_file(path: Path) -> None:
    ...
    if not path.exists():
        return
    # 容忍 .env 被写成带 UTF-8 BOM 的情况（避免第一个键名带上 \ufeff 前缀）。
...
class Settings:
    ...
    # 允许用 .env 的 FRONTEND_DIST 覆盖（改一行 + 重启后端即可切换，不用改代码）。
    ...
    AI_OCR_TOTAL_TIMEOUT = int(os.environ.get("AI_OCR_TOTAL_TIMEOUT", "290"))
